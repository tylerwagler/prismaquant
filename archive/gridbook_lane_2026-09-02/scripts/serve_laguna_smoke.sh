#!/usr/bin/env bash
# ============================================================================
# serve_laguna_smoke.sh — serve the Laguna-S-2.1 6.0bpp gridbook artifact.
# First-boot smoke: NO drafter, fp8 KV, eager. MAXLEN defaults to the full
# 256k mandate; SHORTLEN=1 drops to 16k for the fast first-load smoke (KV
# pool shrinks, slack gate proves the load before committing to 256k).
# Memory math @256k: weights 84 GB + fp8 KV ~24 GiB + engine/acts ~10 GB
# vs 121 GiB pool => start UTIL 0.90, slack-gate + watchdog per the
...
LOG=/home/rob/dq-runs/laguna-s21/logs/serve_smoke.log
mkdir -p /home/rob/dq-runs/laguna-s21/logs
