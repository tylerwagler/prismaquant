...
Default-state variant (gridbook 0.8.9 candidate): applies the same
declared-deviation wrap the measurement shim uses, but the deviations are the
0.8.9 DEFAULTS — PERSISTENT_B=auto, CB_GEMV=auto (FP8_GEMV_V2 is absent from
the gold allowlist and needs none).  Expected on the clean 87 GB body:
    32 FP4-CB routed layers  -> symbol=cb_moe_persistent_b_prefill
    11 FP8-CB routed layers  -> the grouped bridge (per-role split books make
                                the FP8 arm ineligible; auto falls back with
                                the reason in the load log)
...
def main() -> int:
    ...
    gemv = os.environ.get("PRISMAQUANT_CB_GEMV", "")
    print(f"\n=== ROUTE HISTOGRAM (PERSISTENT_B={flag!r} CB_GEMV={gemv!r}) ===")
